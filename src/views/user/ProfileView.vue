<template>
  ...
  <div class="block lg:flex justify-center mt-2 md:mt-0 lg:mt-4 min-h-screen xl:px-12 2xl:px-16 w-full" v-if="user">
    ...
    <div class="block w-auto lg:pr-8 lg:w-1/4">
      ...
      <div
          class="box w-full h-auto mb-2 md:mb-4 flex lg:flex-col flex-none py-2 p-5 md:mt-4 justify-center items-center space-x-4 lg:mt-48 md:space-x-8 lg:space-x-0">
        <div class="hidden md:block font-semibold lg:mb-1 text-xl pr-4">Links</div>
        <div class="flex justify-between py-2"><span class="text-linkedin pr-4">LinkedIn</span><a
            href="https://www.linkedin.com/in/killian-mah%C3%A9-246928135/">killian-mahe</a></div>
        <div class="flex justify-between py-2"><span class="text-github pr-4">GitHub</span><a
            href="https://github.com/killian-mahe">@killian-mahe</a></div>
      </div>
...
export default defineComponent({
  ...
  mounted() {
    API.Notification.get().then(response => {
      API.Notification.markAsRead(response.data.data[0].id);
    })
  },
  computed: {
    ...mapGetters({
