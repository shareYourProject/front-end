<template>
  ...
  <div>
    <!-- Card -->
    <div class="card rounded shadow-md w-full h-auto py-2 items-center" v-if="!resharedPost">
      <div class="flex items-start relative w-full space-x-3" v-click-outside="onClickOutside">
        <div @click="showSelect = !showSelect"
             class="inline-flex h-auto items-center cursor-pointer rounded-md hover:bg-cultured-400 text-onyx-500 hover:text-viridiant-600 py-3 px-2">
          <img class="w-10 h-10 rounded-full object-cover" :src="author.profile_picture" alt="profile_picture">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-1 feather feather-chevron-down" width="24"
        ...
        <div v-show="showSelect"
             class="flex-col space-y-1 flex left-0 top-14 absolute bg-cultured-100 border rounded-md border-gray-300 py-2 px-2 text-sm shadow">
          <div @click="onAuthorSelected(user)"
               class="cursor-pointer justify-start inline-flex items-center space-x-3 hover:bg-cultured-400 py-1 px-2 rounded-md">
            <img class="w-8 h-8 rounded-full object-cover" :src="user.profile_picture" alt="profile_picture">
            <span>{{ user.full_name }}</span>
          </div>
          <div v-for="project in user.owned_projects" :key="'project_'+project.id" @click="onAuthorSelected(project)"
               class="cursor-pointer inline-flex items-center justify-start space-x-3 hover:bg-cultured-400  py-1 px-2 rounded-md">
    ...
    <ModalComponent v-if="showModal || (resharedPost !== undefined)" size="md">
      <template v-slot:header>
        <div class="border-b pb-3 flex justify-between items-center">
          ...
          <svg @click="closeModal" class="feather feather-x cursor-pointer" xmlns="http://www.w3.org/2000/svg"
      ...
      <template v-slot:body>
        <div class="mt-4 flex items-center">
          <img class="w-10 h-10 rounded-full mr-3 inline-block object-cover" :src="author.profile_picture"
               alt="profile_picture">
          <div class="flex flex-col jusitfy-start text-xl">
            <span v-if="author.first_name!=null"
                  class="font-medium">{{ author.first_name }} {{ author.last_name }}</span>
        ...
        <div class="mt-6 max-h-24 overflow-y-auto">
          ...
          <div v-if="files.length > 0">
            <div v-for="(file, index) in files" :key="'file_'+index" class="relative my-2">
              <svg @click="removeFile(index)"
                   class="feather feather-x transition-colors ease-in-out duration-75 absolute top-2 right-2 text-cultured-500 bg-onyx-500 hover:bg-onyx-700 p-1 w-8 h-8 rounded-full cursor-pointer"
          ...
          <input v-if="extraContent" id="pictures_input" ref="picture" @change="previewFile" type="file" name="pictures"
                 hidden>
        </div>
      </template>
      ...
      <template v-slot:footer>
        <div class="w-full flex justify-between items-center mt-2">
          <span v-if="extraContent" @click="selectFile"><svg
              class="feather feather-image hover:bg-onyx-100 bg-cultured-100 p-2 w-10 h-10 rounded-full cursor-pointer transition-colors ease-in-out duration-150"
              xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3"
                                                                                                          ...
                                                                                                          rx="2"
                                                                                                          ry="2"></rect><circle
              cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg></span>
          <span v-else><svg
              class="feather feather-image hover:bg-onyx-100 p-2 w-10 h-10 rounded-full bg-cultured-100 cursor-not-allowed transition-colors ease-in-out duration-150"
              xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3"
                                                                                                          width="18"
                                                                                                          height="18"
