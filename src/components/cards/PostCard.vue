    ...
    <div v-if="post_data !== undefined" class="card md:rounded-lg">
      ...
      <div v-if="post_data.project" class="card-title justify-between">
        <div class="flex items-center">
          <router-link
            ...
            <img class="rounded-full my-auto h-12 w-12 object-cover hover:shadow-md"
                 :src="post_data.project.profile_picture"/>
          </router-link>
          <div class="flex flex-col ml-3 justify-start">
            <router-link class="font-semibold leading-tight"
                         :to="{name:'project', params:{id: post_data.project.id}}">{{ post_data.project.name }}
            </router-link>
      ...
      <div v-else class="card-title justify-between">
        <div class="flex items-center">
          <router-link :to="{name:'profile', params:{id: post_data.author.id}}" class="w-auto inline-grid">
            <img class="rounded-full my-auto h-12 w-12 object-cover hover:shadow-md"
                 :src="post_data.author.profile_picture"/>
          </router-link>
          <div class="flex flex-col ml-3 justify-start">
            <router-link class="font-semibold leading-tight"
                         :to="{name:'profile', params:{id: post_data.author.id}}">{{ post_data.author.first_name }}
              {{ post_data.author.last_name }}
            </router-link>
            <span class="text-xs font-light leading-tight">{{ timeSinceCreation }}</span>
      ...
      <div class="card-body sm:px-0 md:text-lg pt-2">
        ...
        <div class="flex mt-1 mb-2 justify-end">
          ...
          <span
              @click="showComments"
              class="text-sm text-onyx-400 cursor-pointer hover:underline">{{ post_data.stats.comments }} comments</span>
        </div>
      </div>
        ...
        <div class="card-link">
          ...
          <span @click="like(!post_data.liked)" class="cursor-pointer fill-current flex items-center w-10 h-10"
            ...
            <!-- <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="true" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-heart"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg> -->
                      <span class="ml-1 hidden md:inline text-sm font-medium"
                            :class="{'text-red-600': post_data.liked}">{{ post_data.stats.like }}</span>
                  </span>
        </div>
        <div class="card-link">
          <!-- Comments -->
          <span
              @click="showComments"
              class="hover:text-orange-peel-400 cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg"
              ...
              class="ml-1 hidden md:inline">Comment</span></span>
        </div>
        <div class="card-link">

          <span class="hover:text-viridiant-400 cursor-pointer" @click="share()"><svg xmlns="http://www.w3.org/2000/svg"
                                                                                      viewBox="0 0 24 24" fill="none"
                                                                                      stroke="currentColor"
                                                                                      stroke-width="2"
                                                                                      stroke-linecap="round"
                                                                                      stroke-linejoin="round"
                                                                                      class="feather feather-share-2"><circle
              cx="18" cy="5" r="3"></circle><circle cx="6" cy="12" r="3"></circle><circle cx="18" cy="19"
                                                                                          r="3"></circle><line x1="8.59"
                                                                                                               y1="13.51"
                                                                                                               x2="15.42"
                                                                                                               y2="17.49"></line><line
              x1="15.41" y1="6.51" x2="8.59" y2="10.49"></line></svg><span
              class="ml-1 hidden md:inline">Share</span></span>

        </div>
      </div>

    </div>
    <div v-if="display_comments && comments.length > 0"
         class="p-5 mb-2 border-t-0.0625 border-onyx-100 space-y-3 bg-cultured-100">
      <div v-if="isAuthenticated" class="flex relative items-start">
        <img :src="user.profile_picture" class="h-10 w-10 rounded-full mr-3" alt="profile_picture">
        <TextArea class="w-full" child_class="w-full pr-10 overflow-y-hidden resize-none"
                  placeholder="Write a comment..." @send="writeComment" name="comment-content"
                  v-model="newCommentContent" :rows="1"></TextArea>
        <svg class="feather feather-send absolute right-4 top-3 cursor-pointer" @click="writeComment"
      ...
      <CommentCard v-for="comment in comments" :key="comment.id" :comment="comment" class="mt-2"></CommentCard>
      <button v-if="next_comment_page !== -1"
              class="font-sans font-medium text-sm cursor-pointer text-onyx-500 hover:text-onyx-700"
              @click="addComments">Load more comments
      </button>
...
export default defineComponent({
  ...
  directives: {
    ...
    methods: {
      ...
      showComments() {
        if (this.comments.length == 0) this.addComments();
        ...
        API.Post.comments(this.post_data, this.next_comment_page).then(response => {
          switch (response.status) {
            case 200:
              ...
              if (this.next_comment_page == response.data.meta.last_page) this.next_comment_page = -1;
              else this.next_comment_page++;
